...
if (!empty($_COOKIE['shopCookie'])) {
    ...
    if (time() > $t['exp']) $isValid = 0;
    $q = $DBH->prepare('SELECT * FROM users WHERE uemail= ?');
    $q->execute(array($t['em']));

    if ($r = $q->fetch()) {

        //echo gettype($r['upiority']);
        if (time() < $t['exp'])
            echo "   " . $r['upiority'] . "   " . gettype($r['upiority']);
        if (strval($r['upiority']) == "1") {
            $isValid = 1;
        }
    }
}
if ($isValid==0) {
    echo "You are not access to the page.</br>";
    echo "<a href='Shop_host.php'> Go Home Page </a>";
    exit();
}
...
<body>
<fieldset>
    <div> <a href="Shop_host.php"> Home </a>  </div>
    <div> <a href="categories_mng.php"> Categories Admin Panel</a> </div>
