...
?>
<table class="home_log_in" id="log_in_table"><tr><td>
            <?php
            $isValid=false;
            if(!empty($_COOKIE['shopCookie'])) {
                $t = json_decode(stripslashes($_COOKIE['shopCookie']), true);
                if (time() > $t['exp']) $isValid = false;
                $q = $DBH->prepare('SELECT * FROM users WHERE uemail= ?');
                ...
                if($r=$q->fetch())
                {
                    ...
                    {
                        $isValid=true;
                        echo "Welcome ".$r['uname']."</br>";
                        echo "<a href='log_out.php'> [Log Out]</a></br>";
                        if($r['upiority']==1)
                            echo "<a href='categories_mng.php'>phpAdmin panel</a>";
                    }
                }
            if($isValid)
            ...
            <fieldset>
                ...
                <form id="log_in_form" method="POST" action="log_in.php" >
                    <label for="E-mail-input">E-mail *</label>
                    <div><input id="E-mail-input" type="text" name="E-mail-input" required="true"  pattern="^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$" />
                    </div>
                    <label for="password-input">Password *</label>
                    <div>
                        <input type="password" id="password-input" name="password-input" required="true">
                    </div>
            ...
            if($isValid)
                echo "</div>";
            ?>
            <div> Need a new account: click <a href="signUp.html">here</a> to create a new account.</div>
        </td></tr></table>
